...
import { equationAnswerIsExact, formulaElements } from "./equation-policy";
...
type AnswerRecord = { question: EquationQuestion; answers: string[]; correct: boolean };
...
function blankAnswers(question: EquationQuestion, fixedCount: boolean): string[] {
  return Array(fixedCount ? hiddenParticipants(question).length : 1).fill("");
}

function exact(question: EquationQuestion, answers: string[], requireBalancing: boolean): boolean {
  return equationAnswerIsExact(
    answers,
...
function EquationLine({
  ...
  const visibleLine = visible.map((part, index) => (
    <span className="eq-visible-part" key={`${part.side}-${part.position}`}>
      ...
      <Formula value={displayParticipant(part, requireBalancing)} />
    </span>
  ));
  const inputs = (
    <div className="eq-answer-side">
      {answers.map((answer, index) => (
        <span className="eq-answer-slot" key={`${question.id}-${index}`}>
          {index ? <b className="eq-plus">+</b> : null}
          <input
            ref={(node) => { if (node && index === activeInput) node.dataset.active = "true"; }}
            value={answer}
            disabled={submitted}
            ...
            spellCheck={false}
          />
          {!fixedCount && answers.length > 1 && !submitted
...
export default function EquationQuiz({ onSwitchToColors }: { onSwitchToColors: () => void }) {
  ...
  const setNewQuestion = useCallback((nextDirection = direction, nextFixed = fixedCount, reactions = pool) => {
    ...
    setAnswers(blankAnswers(next, nextFixed));
    setActiveInput(0);
    setSubmitted(false);
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      try {
        ...
        if (stored?.version === 1 && stored.question?.reactionId) {
          ...
          if (restoredReaction) {
            ...
            setAnswers(Array.isArray(stored.answers) && stored.answers.length ? stored.answers : [""]);
      ...
      setQuestion(first);
      setAnswers(blankAnswers(first, true));
    }, 0);
  ...
  useEffect(() => {
    ...
    return () => window.clearTimeout(timer);
  }, [answers, data, direction, examConfig, fixedCount, question, requireBalancing, scope, stats]);

  const updateAnswer = (index: number, value: string) => {
    setAnswers((current) => current.map((answer, answerIndex) => answerIndex === index ? value : answer));
    if (mode === "exam_running") {
      setExamAnswers((current) => current.map((answer, questionIndex) =>
        questionIndex === examIndex
          ? answer.map((item, answerIndex) => answerIndex === index ? value : item)
          : answer
      ));
    }
  };
  ...
  const addAnswer = () => {
    ...
    if (mode === "exam_running") {
      setExamAnswers((current) => current.map((answer, index) => index === examIndex ? [...answer, ""] : answer));
    }
    setActiveInput(answers.length);
  };

  const removeAnswer = (index: number) => {
    const next = answers.filter((_, answerIndex) => answerIndex !== index);
    setAnswers(next);
    if (mode === "exam_running") {
      setExamAnswers((current) => current.map((answer, questionIndex) =>
        questionIndex === examIndex ? answer.filter((_, answerIndex) => answerIndex !== index) : answer
      ));
    }
    setActiveInput(Math.max(0, Math.min(activeInput, next.length - 1)));
  };

  const appendToken = (token: string) => {
    if (submitted) return;
    updateAnswer(activeInput, `${answers[activeInput] || ""}${token}`);
  };

  const backspace = () => {
    if (submitted) return;
    updateAnswer(activeInput, (answers[activeInput] || "").slice(0, -1));
  };

  const isCorrect = question ? exact(question, answers, requireBalancing) : false;
  const submitPractice = () => {
    if (!question || submitted || !answers.some((answer) => answer.trim())) return;
    const correct = exact(question, answers, requireBalancing);
    setSubmitted(true);
    setHistory((current) => [...current, { question, answers: [...answers], correct }]);
    setHistoryIndex(history.length);
    setStats((current) => ({
  ...
  const changeFixedCount = (next: boolean) => {
    setFixedCount(next);
    if (question && !submitted) setAnswers(blankAnswers(question, next));
  };
  ...
  const startExam = () => {
    ...
    const answerRows = questions.map((item) => blankAnswers(item, fixedCount));
    const now = Date.now();
    setExamQuestions(questions);
    setExamAnswers(answerRows);
    setExamIndex(0);
    setQuestion(questions[0]);
    setDirection(questions[0].direction);
    setAnswers(answerRows[0]);
    setSubmitted(false);
  ...
  const showExamQuestion = useCallback((index: number) => {
    ...
    setAnswers([...(examAnswers[index] || blankAnswers(next, fixedCount))]);
    setActiveInput(0);
  }, [examAnswers, examQuestions, fixedCount]);
  ...
  const examResults = examQuestions.map((item, index) => ({
    question: item,
    answers: examAnswers[index] || [],
    correct: exact(item, examAnswers[index] || [], requireBalancing),
  }));
  const examCorrect = examResults.filter((result) => result.correct).length;
  const examAnswered = examResults.filter((result) => result.answers.some((answer) => answer.trim())).length;
      ...
      {mode === "exam_setup" ? (
        ...
        <section className="exam-screen result-screen equation-result">
          ...
          <section className="exam-review-list">
            ...
            {examResults.map((result, index) => <details className={result.correct ? "exam-review-card correct-review" : "exam-review-card wrong-review"} key={result.question.id}>
              <summary><span>第 {index + 1} 题</span><b>{result.question.direction === "forward" ? "正推产物" : "逆推反应物"}</b><em>{result.correct ? "正确" : result.answers.some((answer) => answer.trim()) ? "错误" : "未作答"}</em></summary>
              <div className="exam-review-body equation-review-body">
                <p><b>你的答案：</b>{result.answers.filter(Boolean).join(" + ") || "未作答"}</p>
        ...
        <section className="workspace equation-workspace">
          <aside className="control-panel">
            {mode === "exam_running" ? (
              <div className="exam-progress-panel">
                ...
                <div className="exam-question-map">{examQuestions.map((item, index) => <button className={`${index === examIndex ? "current" : ""} ${examAnswers[index]?.some((answer) => answer.trim()) ? "answered" : ""}`} key={item.id} onClick={() => showExamQuestion(index)}>{index + 1}</button>)}</div>
          ...
          <section className="quiz-card equation-card">
            ...
            <EquationLine question={question} requireBalancing={requireBalancing} answers={answers} fixedCount={fixedCount} activeInput={activeInput} submitted={submitted} onInput={updateAnswer} onActivate={setActiveInput} onAdd={addAnswer} onRemove={removeAnswer} />
            ...
            <div className="quiz-actions">
              {mode === "exam_running" ? <><button className="navigation-action" disabled={!examIndex} onClick={() => showExamQuestion(examIndex - 1)}>上一题</button>{examIndex + 1 === examQuestions.length ? <button className="primary-action" onClick={() => { if (window.confirm("确定提交试卷吗？")) finishExam(); }}>提交试卷</button> : <button className="primary-action" onClick={() => showExamQuestion(examIndex + 1)}>下一题</button>}</> : <>
                ...
                {!submitted ? <button className="primary-action" disabled={!answers.some((answer) => answer.trim())} onClick={submitPractice}>确认答案</button> : null}
